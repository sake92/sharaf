...
import org.pac4j.testkit.SessionScenarios
...
class SessionSelfTest extends munit.FunSuite:
  ...
  test("session invalidation") { SessionScenarios.runSessionInvalidation(serverUrl) }
  test("session isolation") { SessionScenarios.runSessionIsolation(serverUrl) }
